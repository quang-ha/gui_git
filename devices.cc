 ...
 *
 */
bool devices::setswitch(namestring sid, asignal level)
{
	devlink d;
	d = netz->finddevice(dvs->lookup(sid));
	bool ok = (d != NULL);
	if (ok) {
		ok = (d->kind == aswitch);
		if (ok)
 ...
 *
 */
bool devices::makedevice (devicekind dkind, name did, int variant)
{ 
	namestring a = (*dvs)[0];
  bool ok = true;
  switch (dkind) {
    ...
    case dtype:
      ...
      break;
    case baddevice:
      throw bad_device_exception("none",0,0);
  }
  return ok;
}
  ...
  do {
    ...
    for (d = netz->devicelist (); d != NULL; d = d->next) {
      switch (d->kind) {
        ...
        case baddevice: throw bad_device_exception("none",0,0);   
      }
      if (debugging)
	showdevice (d);
    }
